...
const parseByOnePeg = (
    ...
    //----------
      ...
      if (token.kind === kind.token) {
        ...
        //----------
          ...
          if (
            grammer[pegCursor - 1].kind === kind.token &&
        ...
        //----------
          ...
          wordLength = 0;
          if (token.token === "@word")
            while (isAlphabet(code[cursors[braceDepth] + wordLength]))
              wordLength++;
          else if (token.token === "@digit")
            while (isDigit(code[cursors[braceDepth] + wordLength]))
              wordLength++;
          else throw "Peg syntax error at " + grammerNumber;
          if (wordLength === 0) setError();
          if (ignore === -1)
            node[braceDepth].push({
              name: name,
              value: code.slice(
                cursors[braceDepth],
      ...
      //----------
        ...
        if (isValidString(token, code, cursors[braceDepth])) {
          if (ignore === -1)
            node[braceDepth].push({ name: name, value: token.token, node: [] });
  ...
  if (packrat && !transient)
    stock[grammerNumber][startLocation] = {
      ast: ast["node"],
      cursor: cursors[0],
    };
  return cursors[0];

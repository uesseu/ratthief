...
ast = peg.parse('1+ 1* 8 / 7', false)
console.timeEnd('No rat test')

console.time('Pack rat test')
peg = new Peg(withWhite)
ast = peg.parse('(1+ 1)* 89 + 9 / 7', true)
ast = peg.parse('1+2+3+4+5', true)
...
ast = peg.parse('aripiprazole, 3, oral', true)
console.log(ast)
